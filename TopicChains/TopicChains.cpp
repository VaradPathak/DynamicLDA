...
double KLDivergence(double** Pi, double* M, int k) {
	double result = 0.0;
	for (unsigned int w = 0; w < W; ++w) {
		result += log(Pi[w][k] / M[w]) * Pi[w][k];
	}
	return result;
}
double JSsimilarity(double** Pi1, double** Pi2, int k) {
	double result = 0.0;
	double* M = new double[W];
	for (unsigned int w = 0; w < W; ++w) {
		M[w] = (Pi1[w][k] + Pi2[w][k]) / 2;
	}
	result = KLDivergence(Pi1, M, k) + KLDivergence(Pi2, M, k);
	result = result / 2;
	return result;
}
